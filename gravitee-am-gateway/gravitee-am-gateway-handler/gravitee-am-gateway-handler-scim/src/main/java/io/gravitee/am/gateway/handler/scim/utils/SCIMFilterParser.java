 ...
 */
package io.gravitee.am.gateway.handler.scim.utils;
...
import io.gravitee.am.gateway.handler.scim.model.Schema;
import io.gravitee.am.gateway.handler.scim.model.filter.AttributePath;
import io.gravitee.am.gateway.handler.scim.model.filter.Filter;
import io.gravitee.am.gateway.handler.scim.model.filter.Operator;
...
public final class SCIMFilterParser {
     ...
     */
    public static Filter parse(final String filterString) throws SCIMException {
        try {
            return new SCIMFilterParser(filterString, Schema.SCHEMA_URI_CORE).readFilter();
        } catch (Exception e) {
            throw new InvalidSyntaxException(MessageFormat.format("Invalid filter ''{0}'': {1}", filterString, e.getMessage()));
        }
    }
